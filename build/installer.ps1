...
if (-not (Test-Path -Path $installtodir -PathType Container)) {
...
& $pipexe install setuptools  pymupdf  pyinstaller --no-warn-script-location
Write-Host "-----------------------------------------------------"
cd ..
$currentdir = $PWD.Path
$programsrc = "src"
...
ps2exe $mainexepath $mainexepath2 -Title "A3 to A4 bulk converter" -Company "diversecellar" -Copyright "Copyright (c) 2025 diversecellar"
